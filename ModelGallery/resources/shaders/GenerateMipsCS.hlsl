...
{
	uint3 GroupID           : SV_GroupID;           // 3D index of the thread group in the dispatch.  ([0-W/8], [0-H/8], 1)
	uint3 GroupThreadID	    : SV_GroupThreadID;	    // 3D index of local thread ID in a thread group. ([0-7], [0-7], 1)
	uint3 DispatchThreadID  : SV_DispatchThreadID;  // 3D index of global thread ID in the dispatch.
	uint  GroupIndex        : SV_GroupIndex;        // Flattened local index of the thread within a thread group. [0-63]
};
...
{
	return select(x < 0.0031308, 12.92 * x, 1.055 * pow(abs(x), 1. / 2.4) - 0.055);
}
