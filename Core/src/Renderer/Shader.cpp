...
{
	std::size_t operator()( const Zenyth::ShaderData& k ) const
	{
		auto seed = std::hash<Zenyth::ShaderStage>()(k.stage);
		seed ^=  std::hash<std::wstring>()(k.path) + 0x9e3779b9 + (seed<<6) + (seed>>2);
		return seed;
	}
};

static std::unordered_map<Zenyth::ShaderData, CompileResult, ShaderDataHasher> s_compilationCache {};
